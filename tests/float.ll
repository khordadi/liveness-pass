...
; Verifies that the result from the liveness for the following module is
; correct. Note that most values are floats, and the popcorn should capture them, too.
...
; CHECK:          %cmp = fcmp ogt float %a, 0.000000e+00
; CHECK-LABEL: BB %if.end8
; CHECK:          %cmp = fcmp ogt float %a, 0.000000e+00
; CHECK-LABEL: BB %if.then2
; CHECK:          %cmp1 = fcmp oeq float %mul, %div
; CHECK:          %cmp = fcmp ogt float %a, 0.000000e+00
